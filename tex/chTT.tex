...
The type-theoretic analogue of the set-theoretic
meta-statement that a given
...
Typing judgments are justified by typing rules, and these
...
logic is seldom mentioned. Second, the term $t$ in a typing
judgment shall represent an
...
\marginpar{Here it is unsatisfactory that you already explained what contexts
are on the previous page, saying that contexts contain typed declarations
and typed definitions, this type, you don't mention typed definitions???}
...
which express subtleties of the type system that are out of scope for
...
$(\C{fun x => t}) u$ into $t[u/x]$:
\marginpar{fonts are not respected here}
 the formal argument
...
\marginpar{Maybe the description of guardedness could be moved from
chapter 1 to here}
...
quantification indicates that \C{ex} and \C{ex_intro} are \C{polymorphic}
...
Hence such a notion of equality needs to be somewhat primitive, as
\C{match} and \C{fun} are.  One also expects such a notion to come
...
two terms of that type (the former is named \C{x} in the
inductive definition).
...
theory; an in-depth study of it is out of scope for this book.  The interested reader
...
While \C{px} has type \C{(P x)}, the function \C{eq_ind} casts it as an
...
possiblity to use a lightweight and compact syntax for this bookkeeping
...
item of an intro pattern, it does not perform a case analysis, but only branches
on the subgoals.
...
is a placeholder for the top item, and
omitting it in \C{/(v _)} leads to \C{/v}.
...
If non-terminating functions were not ruled out, it
would be easy to inhabit
...
The restriction that rejects this kind of data type, called the
...
it by \C{m} in the goal. In our case, this first term is
...
Yet sometimes it is not practical to provide a constructive proof of a
