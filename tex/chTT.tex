...
\subsection{A discussion of total functions}
\label{sec:total}
We are confronted with a striking difference between the usual
...
A function from Section~\ref{ssec:recnat} where the difference is
visible is Function~\C{predn}.  It
...
where the predecessor of \C{0} does exist.
So the predecessor of 3 is a natural number,
...
the necessary suppleness to be able to write mathematical formulas
...
sense, usual mathematical practice prefers to take a risk.  Mathematicians are
...
defined function, a frequent approach is that a neighbour total
function is chosen,
...
This is a departure from usual mathematical practice,
because a given formula has the right value whenever usual
mathematical practice makes it well-defined.  Most mathematical
theorems in the \mcbMC{} library containing formulas with neighbour
total functions will contain premises stating that the formula is well
defined as per usual mathematical practice.  It may take time to accept
such a departure, because mathematical formulas with such neighbour total
function may make stronger claims than what can be read from usual mathematical
perspective, when the premises of theorems are insufficient to guarantee
that formulas can be read just with their meaning in usual mathematics.
...
This frequent approach about total functions is common to many
...
Because we are using type theory, in particular with dependent types, we
have another tool to handle partially defined functions.  Dependent types
make it possible to assert that a function is applied only when the arguments
satisfy the right conditions.  A typical example is given by matrix
multiplication, which is well-defined only for two matrices that have
a dimension in common.  This constraint can be expressed using dependent
types, as was shown in Section~\ref{ssec:terms} and will be illustrated further
in Section~\ref{sec:matrix}.  Functions with dependent type are slightly harder
to use, so that the approach with neighbour total function is often preferred.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
