...
\item \C{EqMixin}, \C{PcanEqMixin}, \lstinline/[eqMixin of T by <:]/:
  ...
  properties the structure provides.\marginpar{The notion of {\sl mixin} was
not sufficiently described before}
...
...
\end{coq}
\marginpar{Too bad this is the only instance of \C{ord0} in the whole book}
...
speeds up type checking, which never requires opening the mixin bundle.
\marginpar{Rather : speeds up later type checking, which never ... ??}
...
is needed for line 7 as only a single property is added.
...
\C{p} is a \C{nat} literal such as 3 or 5.\marginpar{identical \(\rightarrow\) convertible}

Cloning constructors are mainly used to quickly create instances for
defined types, such as\marginpar{This is a long example, you should not
start it by an introduction based on an incomplete sentence}
...
choice, which is derivable in the \mcbCIC{}% \footnote{behind the
...
\gotcha{If you see an undocumented \lstinline/GRing.something/,
  then you have broken an abstraction barrier}\marginpar{An example would be nice}
...
provide explicitly because it is intended that they should always be inferred.
...
matching and transport\marginpar{Do you mean transport in the sense
of HOTT? Was this notion introduced before?
I think this paragraph deserves an illustrating example},
the complexity of doing so can be daunting.
...
\marginpar{I wondered where does \C{m} come from? thinking it had to be part of the previous paragraph}
...
can put \C{windrose} in bijection with a pre-existing finite type,
...
function.\marginpar{Makes no sense to me}
...
\end{coq}
After that, our new data type can be used exactly like the ordinal one
can.
