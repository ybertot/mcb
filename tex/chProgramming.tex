...
\begin{coq-left}{name=fail_check_repeat_twice_f}{width=5.5cm,title=Ill-formed
  application}
...
Coq'Art book \cite{BC04}.\marginpar{Discussion Enrico+Yves, 28/03/18: explanations about logical constraints to data-types programming should go in a separate section, where predn and subn will be defined if not already: topics should
be exhaustiveness and recursive guard checking}.
...
\marginpar{discussion Enrico+Yves 28/03/18: this paragraph about notations
for natural numbers should go in the section about notations}
...
one rewrite the definition above as follows:\marginpar{if the name of the
constructor is not relevant, why do we have to write it down?  I
would rather remove this remark about a name that is not relevant.}
...
depend on the iterated function. By assuming further properties
