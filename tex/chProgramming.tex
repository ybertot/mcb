...
what the \C{(addn n m)} program does is simply to stack \C{n} times the
...
As a third example, we define a function that serves the double purpose
of computing subtraction and comparing two numbers in the sense 
{\em larger than or equal}.  The description of this function is reminiscent
of the equality test function, in the sense that it contains pattern-matching
on the two inputs, but it returns a natural number, which is what remains in
the first argument when either the first or the second argument is exhausted.
In fact, the returned value is 0 when the second argument is larger than or
...
The \mcbMC{} library contains a similar definition, together with a notation
definition so that \C{sub n m} is written \C{n - m}.  This is sensible because
the result
of \C{subn} is the subtraction whenever the minuend (i.e. \C{n}) is
larger than the subtrahend (i.e. \C{m}).  The library also contains
a definition of comparison based on \C{subn}.
\begin{coq}{name=leq}{}
...
A fourth example of recursive function is a function that tests
whether its argument is odd.

\begin{coq}{name=odd}{}
Fixpoint odd n := if n is p.+2 then odd p else (n == 1).
\end{coq}
This definition is interesting because it illustrates the fact
that the argument to the recursive call can be another integer than
the predecessor of the initial argument.  Here, the initial argument
is \C{n} and the argument to recursive call is \C{p}, the predecessor of the
predecessor of \C{n} when it exists.
