...
The rules of the \mcbCIC{}, as sketched in~\ref{ch:ttch},
are expressed on the syntax of terms and
are implemented by the kernel of \Coq{}.  This software component
performs \emph{type checking}: given a term and type it checks if such
a term has the given type.  To keep type checking simple and decidable
the syntax of terms makes all information explicit. As a consequence
the terms written in such a verbose syntax are pretty large.
...
thanks to some background knowledge.
...
special placeholder ``\lstinline/_/'', usually called an \emph{implicit
argument}, may occur inside types and stand for one, and only one,
...
either, since the good value may not exist.  Just think about the
...
language.  We refer to such a language as the language of
...
$x$ is a unique name).\marginpar{this is a convention just for this chapter?}
...
In the light of that, we observe that every time we apply the identity
...
This phenomenon is so frequent that a user can ask the system to insert
the right number of \lstinline/_/ for her.  For more details
...
\lstinline/pr_q/ term.  Such a term has its type fixed by the context:
...
\lstinline/Pack/ with two arguments.  The first argument
is named \lstinline/sort/ and holds a type; the second 
...
\footnote{The meaning of a symbol in mathematics is even deeper: by
	...
	ring one talks about, recall its theory, and use this
...
\begin{coq}{name=parsenocs}{width=4cm}
Check (3 == 4).
$~$
\end{coq}
\begin{coqout}{run=nc}{width=8cm}
...
function (more precisely a record instance containing a comparison
function).
...
(at most) one \emph{canonical} comparison function.  Once these
commands have been executed, the type inference algorithm is able to
proceed.

\begin{coq}{name=parse}{width=6cm}
Check (3 == 4).
Check (true == false).
Eval compute in (3 == 4).
\end{coq}
\coqrun{name=cs}{ssr,eqtype,eqn,bety,infix,declcs,parse}
\begin{coqout}{run=cs}{width=6cm}
3 == 4 : bool
true == false : bool
= false : bool
\end{coqout}
...
adds to this table an entry for each field of the record type.
...
the functionalities tied to the interface to work on the new concept.
...
of the record. Exercise~\ref{ex:triple} proposes to implement the
...
End Equality.
\end{coq}
\marginpar{there is an inconsistency here: the equality is
named \C{op} in the Structure, but used as \C{eq_op} in the
axiom}
...
\C{type}, \C{sort},\marginpar{further inconsistency: we use name \C{eq}
now, but \C{Equality.op} just below}
...
among all instances of each interface.
...
The concept of interface is crucial to
...
a proof of the \C{reflect} inductive spec; a case analysis on the term
 \C{(x1 =P x2)} thus has two branches. Since the constructors
...
As an example we build a sequence of sequences, and we verify that
...
a finite domain.  In particular such a finite domain can be represented
by a list; the general term $f(i)$ by a function \C{(fun i =>
...
all components\marginpar{I am disturbed by the fact that the notion
of predicate \C{pred} was not introduced before}
...
If we chain the two lemmas we can pull out the last item.\marginpar{This looks incorrect, lines 1 and 2 have no instance of \C{odd}}
...
Note that wherever the operation \C{op} occurs, 
the \C{Monoid.operator} projection is being used.
...
being iterated on is \C{nil}, then the result is the initial value.  Such
...
Qed.
\end{coq}
\begin{coqout}{run=roar}{title=Response after line 6,width=6.9cm}
F : nat -> nat
l1, l2 : seq nat
============================
addn_monoid
...
It is not uncommon to see the \C{/=} switch in purely algebraic proofs
...
types used to trigger canonical structure resolution.\marginpar{The transition is too weak: I don't see why we need to trigger canonical structure resolution
to solve the problem described in the previous paragraphs}
...
unification, which in turn is used to compare types.  Types are
...
and puts it into a type.  We will then wire things up so that such a
...
\marginpar{This whole section feels way too terse.  A paragraph at the beginning discussing occasions where the problem occurs could interesting.  Another paragraph at the end showing instances of usage in mathcomp would also be good.}
