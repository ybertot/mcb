...
for instance for an abstract sequence \C{s}.
...
Qed.
\end{coq}
\marginpar{some text is needed here to explain where this goal appears
in the history}
...
in conjunction with \C{iffP}.\marginpar{Why seldom?  It feels strange that
we choose an example that makes us see a technique that is seldom used}
...
exercise~\ref{ex:eqnPinj}.\marginpar{Here, I am quite disturbed by the
fact that this example relies precisely on the fact that arguments
of \C{eqnP} have been declared implicit, but this is not explained}
...
in the middle of intro-patterns.\marginpar{I disagree with the use of
the colon in this sentence: it gives the impression that
using views lemmas in the middle of intro-patterns just happened, while
it is only going to be explained in the next section}
...
inserted a term, called a \emph{hint view}, which plays the role of an
adapter.
...
by cases on a term of type \C{(reflect P b)}, one obtains two proof
...
Remark how the automatic substitution makes the
second goal trivial.  The first one can be solved by replacing
...
use, especially for ``spec'' lemmas.

If one needs to override the pattern inferred for the index
of \C{andP}, one can provide the pattern by hand as follows:
...
of the techniques and tactics seen so far to prove that the
Euclidean division algorithm is correct.  The three subsections contain
...
that: every natural number greater than 1 has at least one prime divisor.  A
 prime divisor $p$ of $m! + 1$ can be shown to be greater than $m$ as
...
The next step is to use the \C{pdivP} lemma to obtain a prime divisor of
...
then prove one case where we pick $x$ to be smaller than $y$.
...
and \C{||}; and the final proof, by transitivity, in the case where
...
as we did for the postage stamp example in section~\ref{sec:strongind}.
...
and makes the invariant of the division loop explicit, by replacing
\C{m} with \C{(0 * d.+1 + m)}.  Recall the case \C{d} being \C{0} has
...
\marginpar{This sentence makes no sense to me}
...
These tasks are mainly implemented with programming type inference.\marginpar{as explained where?}
