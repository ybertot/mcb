...
Inductive data types have both been used to code data, like lists, and
...
easy invariant and relieve the proof text from boring details.
...
proof \C{p}, one recovers a number of type \C{nat} that can be passed to, say, the
...
object itself.
We call such a construction a \emph{sub-type}.

Such a representation can be expensive in the sense that it imposes
extra work (proofs!) to create a sub-type element, so it must be used
with care.  The \mcbMC{} library provides several facilities that
support the creation of record-based sub-types and their
...
once again the know-how of a trained reader.
...
\marginpar{Maybe useful to not that \C{tval} is a coercion}
...
The head symbol of both terms is \lstinline/size/, but then
...
of this section's approach is that the user
...
whenever \C{T} is.  We now want to transport this \C{eqType} structure
...
The proof state after line 3 is displayed below.
The first direction is trivial by rewriting.
The converse direction makes an essential use of
the \lstinline/eq_irrelevance/ lemma, which will be discussed
...
the proof state after line 6 is displayed below.
...
that equip \C{eqType} is available on tuples.
...
pattern} advocated by Chlipala~\cite[section 8.4]{CPDT}.
...
an enumeration for sub-types needs to be defined.
...
all the theory that equip T is also available on ST.  Aliasing a
type is useful to attach different notations or coercions.
...
that proves such a property for a wide class of predicates.
...
Forming sub-types adds no complication from a logic perspective since
...
structure of types equipped with such an enumeration, that any \C{finType}
...
accessors only accept inhabitants of a finite type of size $m$.
...
\marginpar{It is too difficult to know where this proof state comes from}
...
\C{(reflect ([seq x <- s | a x] = s) (all a s))} holds for any
...
inhabitant of a \C{finType} in its enumeration.\marginpar{This is too terse, an example would be welcome.}
...
considered equal.  This principle, that we call \emph{functional
...
such as Cubical Type Theory~\cite{cubicaltt}, include such a
principle.
...
the type of finite function by simply giving the type of the domain and
...
but the term \C{\{ffun nat -> nat\}} would raise an error message, since
there cannot be a registered instance of finite type for \C{nat}.

Other utilities make it possible to apply a finite function as a regular
function or build a finite function from a regular function.
...
What \C{codom\_tuple} builds is a list of values that \C{f} takes
...
lemma.\marginpar{The transition is brutal.  Why not add a subsection?}
...
Such a lemma, rephrased in mathematical notation down below,
...
functions from \C{I} to \C{J} is formed in order to
...
We have seen how sub-types let users easily define a new type by, typically,
...
subjects of study are sets and subsets of the type's inhabitants.
In such case, it is rather inconvenient to define a new type for
each subset, because one typically want to combines elements of
...
subsets that play an instrumental role when studying finite groups.
...
finite functions.\marginpar{What do you mean {\sl omit}?  Isn't it rather that we don't give details, but the trick is also used here?}  Finite sets do validate extensionality and
...
on simpler structures that we did not detail for conciseness.  In
...
We will describe such a type only in the next chapter.  From now
...
determinant.  We base it on Leibniz' formula:
...
First, the type of the inputs makes such an operation total, i.e., \Coq{}
...
\C{castmx} operator and all its companion lemmas make it possible
...
from the one found in the \mcbMC{} library in a tiny detail.
